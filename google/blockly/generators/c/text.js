...
Blockly.C['text_length'] = function(block) {
  ...
  return [text + '.length', Blockly.C.ORDER_MEMBER];
};
...
Blockly.C['text_isEmpty'] = function(block) {
  ...
  return ['!' + text + '.length', Blockly.C.ORDER_LOGICAL_NOT];
};
...
Blockly.C['text_charAt'] = function(block) {
  ...
  var textOrder = (where == 'RANDOM') ? Blockly.C.ORDER_NONE :
      Blockly.C.ORDER_MEMBER;
  var text = Blockly.C.valueToCode(block, 'VALUE',
      textOrder) || '\'\'';
  switch (where) {
 ...
 */
Blockly.C.text.getIndex_ = function(stringName, where, opt_at) {
  if (where == 'FIRST') {
    return '0';
...
Blockly.C['text_getSubstring'] = function(block) {
  ...
  if (where1 == 'FIRST' && where2 == 'LAST') {
    ...
    var getIndex_ = Blockly.C.text.getIndex_;
...
Blockly.C['text_changeCase'] = function(block) {
  ...
  var textOrder = operator ? Blockly.C.ORDER_MEMBER :
      Blockly.C.ORDER_NONE;
...
Blockly.C['text_trim'] = function(block) {
  ...
  var text = Blockly.C.valueToCode(block, 'TEXT',
      Blockly.C.ORDER_MEMBER) || '\'\'';
  return [text + operator, Blockly.C.ORDER_FUNCTION_CALL];
};
